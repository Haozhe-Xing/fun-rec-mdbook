...
**扩散模型（Diffusion Model）** 凭借其「先逐步加噪、再学习去噪」的生成范式，为这些问题提供了独特工具。它不像判别式模型去「打分」，也不像自回归模型去「逐 token 生成」，而是在连续潜在空间里通过多步去噪「雕刻」出目标。本章沿两条主线展开：一是**数据增强**（用扩散生成高质量伪交互/跨场景样本），二是**特征增强与多样性优化**（用扩散补全缺失特征、生成多样化 slate）。
