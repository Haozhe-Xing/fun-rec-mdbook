...
本部分以刘鹏《计算广告学》公开课体系为骨架、近年 KDD/SIGIR/RecSys 工业论文为延伸，为推荐系统背景的读者补齐计算广告知识：先俯瞰**广告投放模式的演进与程序化生态**（12.1），再拆解**计费模式与核心指标**这根贯穿广告系统的主线（12.2），然后深入本部分的压轴主题——**竞价机制**（12.3）：为什么一价拍卖让市场震荡，为什么广义第二高价能长期成为工业主流，VCG 的「说真话」又代价几何。最后两章把镜头拉回工程侧：**智能出价与预算控制**（12.4）讲平台如何替广告主出价、如何平滑花预算、如何在一价市场里 bid shading；**偏差与校准**（12.5）讲广告预估为什么必须「绝对准」而不只是「排序对」，以及位置偏差、样本选择偏差的治理与工业校准体系。其后的**开环广告与闭环广告**（12.6）从数据可观测性视角收束实效广告：转化是否发生在平台可观测的域内，决定了平台能做多深的优化。其后**在线分配与流量管理**（12.7）补上合约广告的算法底座——供给/需求二部图、紧凑分配方案与 HWM，这套「约束优化 + 对偶定价」框架也是 12.4 预算出价的理论源头。接下来的四章把知识库补齐到从业者全景：**受众定向技术**（12.8）讲 t(c)/t(u)/t(a,u) 三类标签体系与行为定向建模；**广告检索与语义召回**（12.9）讲布尔索引、WAND 剪枝与 ANN 语义召回的检索漏斗；**数据加工与交易**（12.10）讲三方数据、DMP/CDP 与隐私合规；**实验框架与反作弊**（12.11）以「度量可信 + 流量真实」两条底线守住系统。末尾两章回补产品形态主线：**合约广告**（12.12）讲清「先有广告位后有广告」的售卖演进——CPT/CPD 排期、从卖位置到卖人群，商业逻辑正是 12.7 在线分配算法的前置；**信息流与原生广告**（12.13）讲广告与内容的融合——信息流混排、激励视频、oCPX 产品链路，这是与推荐工程结合最深的一章。
