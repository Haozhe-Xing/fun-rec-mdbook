...
> 📝 **Before You Continue:** 请先读完 [Part 3 排序](../) 的打分函数 $f$，理解精排如何为每个候选输出一个相关性分数（如 CTR 预估分）。本章正是站在精排之后，对那份「按分数排序」的列表做最后一里路的优化。
