...
- **LTV 出价（LTV Bidding）** — 以用户生命周期价值替代单次转化价值作为出价锚点的形态：$\widehat{\text{LTV}}(u)$ 通常拆成「留存概率 × 每期价值」两段建模再相乘，应对重尾稀疏分布。
...
_术语表覆盖上篇（Part 1–5）、下篇（Part 6–11）与专题（Part 12 计算广告）全书内容。__
