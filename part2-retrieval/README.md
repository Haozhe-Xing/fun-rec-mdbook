...
本部分沿技术演进展开五个章节：从最经典的**协同过滤**出发，到把序列建模思想迁移进推荐的**向量召回（I2I）**，再到用深度网络实现高效检索的**双塔模型（U2I）**，继而关注被前几种方法忽略的**序列/时序信息**（序列召回），最后跳出「模型内部压缩」的思路，用**流式索引**保留全量历史兴趣。它们共同构成了工业界召回层的方法图谱。
...
- 🟢 **解释** Swing 如何利用二部图结构过滤噪声，以及 EGES 如何用眼注意力解决冷启动
